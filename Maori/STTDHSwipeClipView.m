...
@implementation STTDHSwipeClipView
...
//        NSScrollView *scrollView = [[[[webView mainFrame] frameView] documentView] enclosingScrollView];
        ...
        if(bounds.origin.x + bounds.size.width >= [webView bounds].size.width)
        {
...
//        appDelegateObject.panelController.hasActivePanel = appDelegateObject.menubarController.hasActiveIcon;
        ...
        {
            [appDelegateObject nextTrack:nil];
        }
        isHandlingEvent = NO;
        if(currentSum != 0)
        {
    ...
    else if([event phase] == NSEventPhaseBegan || [event phase] == NSEventPhaseCancelled)
    {
        isHandlingEvent = NO;
        if(currentSum != 0)
        {
