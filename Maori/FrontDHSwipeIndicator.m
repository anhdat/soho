...
@implementation FrontDHSwipeIndicator
...
//        NSScrollView *scrollView = [docView enclosingScrollView];
        self.clipView = [[FrontDHSwipeClipView alloc] initWithFrame:[webView frame] webView:webView];
...
- (void)drawRect:(NSRect)dirtyRect
{
    if(clipView.currentSum != 0 && clipView.isHandlingEvent)
    {
        ...
        if(sum < 0)
        {
            drawRect = NSMakeRect(0-(49-49*percent), self.frame.size.height/3, 49*percent, 100);
